...
class Simulator(object):
    ...
    def __run_sims_impl(As, Ps, Bs, epoch=None):
        ...
        for i in range(N):
            ...
            # Find the date-ranges of the simulation.  We propagate
            # forward by the time-step of the output (usually one day
            # so that our final numbers are delta Apogee/Perigee over
            # one day) then have a more fine-grained set of values
            # over the course of a single orbit.  That way we can
            # determine the actual Apogee and actual Perigee over the
