...
def vary_raan(A, P, inc, dt_min=1, N=360, ax=None):
    ...
    print(f"=== Examining Effects of RAAN/Inc on Apogee/Perigee ===")
    print(f"  a: {a} km")
    print(f"  b: {b} km")
    print(f"  c: {c} km")
    print(f"  A: {A} km")
...
def vary_day(A, P, inc, dt_min=1, N=360, ax=None):
    """Check the impact of time of the month.
...
def vary_day(A, P, inc, dt_min=1, N=100, ax=None):
    """Check the impact of the lunar cycle
    ...
    # When else would we choose?
    start_epoch = datetime.datetime(1955, 11, 5, 0, 0, 0)

    # The RAAN values to loop through
    day = datetime.timedelta(days=1)
    epochs = np.arange(start_epoch, start_epoch+N*day, day)
    ...
    print(f"=== Examining Effects of Lunar Cycle on Apogee/Perigee ===")
    print(f"  N: {N} days")
    print(f"  a: {a} km")
    print(f"  b: {b} km")
    print(f"  c: {c} km")
    ...
    # Loop through the raans and run a single sim
    for i in range(N):
        epoch = start_epoch + datetime.timedelta(days=i)
        ...
        Aidx = np.argmax(R)
        assert(R[Aidx] == np.max(R))
        Pidx = np.argmin(R)
        assert(R[Pidx] == np.min(R))

        Ap_s[i] = Alt[Aidx]
        Pp_s[i] = Alt[Pidx]

    if ax:
        ax.set_xlabel("Cycle Position (days)")
        ax.set_ylabel("Orbital Altitude (km above MSL)")
        CA = 'firebrick'
        CP = 'dodgerblue'
        X = np.arange(0, len(epochs), 1)
        ax.plot(X, Ap_s, color=CA, label='Apogee')
        ax.plot(X, Pp_s, color=CP, label='Perigee')
        ax.set_title(f"A={int(A)} P={int(P)} inc={int(inc)}")

    return epochs, Ap_s, Pp_s
...
def plot_day(A=600, P=500):
    fig = plt.figure(figsize=(12, 8), dpi=600)

    fig.suptitle(f"Impact of Lunar Cycle on effective vs nominal Apogee/Perigee")

    vary_day(A, P, 0, ax=fig.add_subplot(2, 2, 1))
    vary_day(A, P, 30, ax=fig.add_subplot(2, 2, 2))
    vary_day(A, P, 60, ax=fig.add_subplot(2, 2, 3))
    vary_day(A, P, 90, ax=fig.add_subplot(2, 2, 4))

    fig.tight_layout(h_pad=2)

    fig.savefig(f"lunar.png")

if __name__ == '__main__':
    plot_raan_inc(A=700, P=700)
    plot_day(A=950, P=850)
